gran.stats <-function(data, output = "phi", method="folk", verbal=FALSE, lang="en-US"){
  ...
  #Adaptação para tirar base::order, substituindo por plyr::arrange, segundo
  #https://win-vector.com/2021/02/07/it-has-always-been-wrong-to-call-order-on-a-data-frame/  
  ...
  dd=data.frame(t(data))
  dd=arrange(dd,dd[,1])
  tab.ver=data.frame(t(dd))
    if (sum(data[1,]) > 550){ #se a soma das classes for maior que 550 entao as classes nao estao em phi 
      tab.ver[1,]<-(-log2(tab.ver[1,]/1000)) #transforma em phi
      dd=data.frame(t(tab.ver))
      dd=arrange(dd,dd[,1])
      tab.ver=data.frame(t(dd))
    }
